...
d <- ggplot(f, aes(observation_date, frame = frame)) +
  ...
  annotate("text", label = "Tax Cut Approved", x = plot$observation_date[15], y = 0, color = "black", size = 4.5) +
...
``` {r include = FALSE, message = FALSE, echo = FALSE}
p <- ggplotly(d, ) %>%
  layout(legend=list(y = -0.75, orientation = "h",  
                     xanchor = "center",  
                     x = 0.5)) %>%
   animation_opts(
    frame = 100,
...
htmlwidgets::saveWidget(p, "manlike.html")


```

<iframe src=/content/post/manlike.html width=100% height="400"</iframe>
